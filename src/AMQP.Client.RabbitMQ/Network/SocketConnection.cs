...
{
    internal class SocketConnection : Connection, IAsyncDisposable
    {
        ...
        internal SocketConnection(ILogger trace, Socket socket, MemoryPool<byte> memoryPool, PipeScheduler scheduler, bool waitForData = true)
        {
            ...
            var pair = DuplexPipe.CreateConnectionPair(memoryPool as RabbitMQMemoryPool);
        ...
        public override IConnectionProperties ConnectionProperties => null;
        ...
        private async Task DoReceive()
        {
            ...
            catch (SocketException ex) when (IsConnectionResetError(ex.SocketErrorCode))
            {
                ...
                if (!_socketDisposed)
                {
                    _trace.LogInformation("Socket disposed");
                }
            }
            catch (Exception ex)
        ...
        private async Task ProcessReceives()
        {
            ...
            while (true)
            {
                ...
                {
                    // FIN
                    _trace.LogInformation("ProcessReceives fin");
                    break;
                }
                ...
                if (paused)
                {
                    _trace.LogInformation("Socket paused");
                }
        ...
        private async Task DoSend()
        {
            ...
            catch (SocketException ex) when (IsConnectionResetError(ex.SocketErrorCode))
            {
                ...
                _trace.LogInformation($"{nameof(SocketConnection)} shutdow with {ex.Message}");
                _trace.LogInformation($"{nameof(SocketConnection)} shutdow with {ex.StackTrace}");
            }
            catch (Exception ex)
                when ((ex is SocketException socketEx && IsConnectionAbortError(socketEx.SocketErrorCode)) ||
                       ex is ObjectDisposedException)
            ...
            catch (Exception ex)
            {
                shutdownReason = ex;
                unexpectedError = ex;
                _trace.LogError($"{nameof(SocketConnection)} shutdow with error {unexpectedError.Message}");
                _trace.LogError($"{nameof(SocketConnection)} shutdow with error {unexpectedError.StackTrace}");
            }
            finally
            {
        ...
        private void Shutdown(Exception shutdownReason)
        {
            lock (_shutdownLock)
            {
                ...
                _trace.LogInformation(_shutdownReason.Message);
